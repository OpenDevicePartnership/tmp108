...
device description (`tmp108.ddsl`) by the
...
```
.
├── Cargo.toml              # MSRV 1.94; edition 2024; deny-list lints
├── tmp108.ddsl             # device-driver register description (DDSL)
├── CHANGELOG.md            # Keep-a-Changelog v1.1.0 format
├── src/
│   ├── lib.rs              # Tmp108 + AsyncTmp108 + AlertTmp108 +
│   │                       # Config + Error + private mod ops + traits
│   └── inner.rs            # GENERATED by device-driver from tmp108.ddsl — do not hand-edit
...
`src/inner.rs` is machine output, committed to the tree. There is no
`build.rs` and no `create_device!` macro; regenerate it out-of-band
with the `device-driver-cli` (2.1.0) binary:
...
Modify `tmp108.ddsl` for register-layout changes; everything else
flows from there.
...
| A new register field | `tmp108.ddsl`, regenerate `src/inner.rs` with `ddc build rust -s tmp108.ddsl -o src/inner.rs` |
